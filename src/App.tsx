...
function App() {
  ...
  const [candidate2, setCandidate2] = useState<(string|number|string[])[][]>([]);
  ...
  return (
    <div className="App" lang='ja'>
      ...
      {iterable.map(i => (
        <Linebox top1={position[vias[i]-1][0]} top2={position[vias[i+1]-1][0]} left1={position[vias[i]-1][1]} left2={position[vias[i+1]-1][1]} time={(times[vias[i]-1].filter(e => (e as number[])[0] === vias[i+1])[0] as number[])[1]}></Linebox>
      ))}
      
      {array.map(i => (<Station stress1={[can,dep,ter].includes(i)}  stress2={vias.includes(i)} top={position[i-1][0]} left={position[i-1][1]} name={stations[i-1][1] as string} pos={pos[i-1]} stationNumber={stations[i-1][3] as string[]}/>))}
      
    
      
      <div className='depbox' style={{backgroundColor: 'skyblue'}}>
        <h4 className='dep'>出発駅</h4>
        <TextField label='出発駅' onChange={(e) => {setA(e.target.value);
          setCandidate(stations.filter((f) => (f[1] as string).startsWith(e.target.value.replace(/[ｂ-ｚ]/g,'')) || (f[2] as string).startsWith(e.target.value.replace(/[ｂ-ｚ]/g,''))))}}
          value={a} width='222px' height='35px'></TextField>
        {(a && candidate.length !== 0) && candidate.length <= 5 && (
          <div className='dep'>
            もしかして
            {candidate.map(e => (
              <Button key={e[0] as string} onMouseEnter={() => setCan(e[0] as number)} onMouseLeave={() => setCan(0)} color='primary' onClick={
                () => {setDep(e[0] as number); setA(e[1] as string); setCandidate([])}
              }>{e[1]}</Button>
            ))}
          </div>
        )}
        {/* {a && candidate.length === 0 && <div className='dep' style={{color: 'red'}}>候補なし</div>} */}
      </div>
      <div className='depbox' style={{backgroundColor: 'pink'}}>
        <h3 className='dep'>到着駅</h3>
        <TextField label='到着駅' onChange={(e) => {setB(e.target.value);
          setCandidate2(stations.filter((f) => (f[1] as string).startsWith(e.target.value.replace(/[ｂ-ｚ]/g,'')) || (f[2] as string).startsWith(e.target.value.replace(/[ｂ-ｚ]/g,''))))}}
          value={b} width='222px' height='35px'></TextField>
        {b && candidate2.length !== 0 && candidate2.length <= 5 && (
          <div className='dep'>
            もしかして
            {candidate2.map(e => (
              <Button key={e[0] as string} onMouseEnter={() => setCan(e[0] as number)} onMouseLeave={() => setCan(0)} color='error' onClick={
                () => {setTer(e[0] as number); setB(e[1] as string); setCandidate2([])}
              }>{e[1]}</Button>
            ))}
          </div>
        )}
        {/* {b && candidate2.length === 0 && <div className='dep' style={{color: 'red'}}>候補なし</div>} */}
      </div>
      
      {dep && ter && (
      <div style={{position: 'absolute', top: 260, left: 20, width: 900}}>
        <h2> {stations[dep-1][1]}→{stations[ter-1][1]} <span style={{fontSize: 15}}>所要時間</span>{time}分</h2>
